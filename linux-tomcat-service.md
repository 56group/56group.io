...
```
shell> useradd -d /usr/local/tomcat -s /bin/bash tomcat
shell> cd /tmp
shell> wget TOMACAT_DOWNLOAD_ADDR
shell> tar zxvf apache-tomcat-VERSION.tar.gz -C /usr/local/
shell> ln -s /usr/local/apache-tomcat-VERSION /usr/local/tomcat
shell> chown -R tomcat /usr/local/tomcat
```
...
shell> netstat -npl | grep 8080 # ten second after
```
...
```
shell> cd /etc/init.d
shell> vi tomcat # 添加tommcat-init.sh内容
shell> chmod u+x tomcat
